# This script reads in the raw ECG data, finds the peaks for each channel, and calculates the average beat
# for each channel. The average beats are saved to a new array. The script also saves metadata
# about the average beats to a CSV file.

# The script does not have backward compatibility. All the data files generated will have a file number
# appended to the file name. For example, the average beat array for file 16 will be saved as
# one_beat_array_16.npy. The metadata for file 16 will be saved as # average_beat_metadata_16.csv.
# The peak summary for file 16 will be saved as peak_summary_16.csv. Earlier the files were saved
# without _n in the filenames.

# The script will process all the files from 1 to 17
...
def one_beat(df, n=16):
    """
    Find peaks for each channel; find the mode number of peaks across channels; for each channel with the mode number
    of peaks, find the average interbeat interval; retain subjects with at least 9 channels with the same number of
    peaks, an average interbeat interval less than the 90th percentile, and a heart rate between 50 and 120. For each
    retained subject, find the average beat for each channel and save to a new array.
    """
    ...
    for subject in range(data_array.shape[0]):
        ...
        for channel in range(12):
            ...
            exam_id_column.append(exam_ids[subject])
    summary_frame['peaks'] = peak_list
    summary_frame['exam_id'] = exam_id_column
    ...
    summary_frame.to_csv(f"{DATA_DIR}/peak_summary_{n}.csv", index=False)
    ...
    np.save(f"{DATA_DIR}/one_beat_array_{n}.npy", one_beat_array)
    ...
    df.to_csv(
        f"{DATA_DIR}/average_beat_metadata_{n}.csv",
        index=False
    )
...
if __name__ == "__main__":
    for n in range(1, 18):
        ...
        if Path(filename).exists():
            ...
            print(f"File number {n} does not exist. Skipping.")
